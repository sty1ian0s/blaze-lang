# Phase 4 – Ecosystem Crate: `blaze‑midi`

> **Goal:** Provide a data‑oriented, pure‑Blaze library for reading, writing, and manipulating MIDI data.  It supports Standard MIDI Files (SMF), real‑time MIDI I/O via platform‑specific backends, and a simple event‑based composition API.  All parsing and writing operations are pure; real‑time I/O carries the `io` effect.  The crate is designed for music software, game audio, and embedded synthesizers.
...
All types are linear where they own resources (files, ports); events and tracks are plain data structs (often `@copy` for small items).  The crate uses `blaze‑serde` for optional serialization of MIDI data to/from a custom binary format and `blaze‑time` for timestamps.
