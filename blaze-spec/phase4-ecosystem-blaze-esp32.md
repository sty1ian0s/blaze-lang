...
- **`WiFi`** – a high‑level abstraction over the ESP32’s built‑in Wi‑Fi stack, using the `esp‑wifi` library.  Wi‑Fi operations require the `hal` effect and are asynchronous (they use the actor model to handle events).
